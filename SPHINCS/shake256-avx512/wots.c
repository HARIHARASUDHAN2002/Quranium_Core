        ...
        uint32_t addr[8])
{
    ...
    /* set addrs = {addr, addr, addr, addr,addr, addr, addr, addr} */
    for (j = 0; j < 8; j++) {
    ...
    for (i = 0; i < SPX_WOTS_LEN; i += 8) {
        ...
        for (k = 0;; k++) {
            ...
            thashx8(bufs[0], bufs[1], bufs[2], bufs[3],bufs[4], bufs[5], bufs[6], bufs[7],
                    bufs[0], bufs[1], bufs[2], bufs[3],bufs[4], bufs[5], bufs[6], bufs[7], 1, pub_seed, addrs);
        }
    }
}
