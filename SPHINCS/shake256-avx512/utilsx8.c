 ...
 * idx, addr_idx on the gen_leafx8 call if tree_height < 2 would fix it; since
                ...
                void *info)
{
    ...
    unsigned char stackx8[tree_height*8*SPX_N];
    ...
    for (idx = 0;; idx++) {
        ...
        gen_leafx8( current, sk_seed, pub_seed, 8*idx + idx_offset,
                    info );
