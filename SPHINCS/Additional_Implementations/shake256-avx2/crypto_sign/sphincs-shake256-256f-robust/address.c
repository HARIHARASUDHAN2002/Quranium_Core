...
void set_tree_addr(uint32_t addr[8], uint64_t tree)
{
    ...
    // Use a larger data type for larger tree sizes
    uint8_t tree_bytes[8];
    ull_to_bytes(tree_bytes, 8, tree);
    memcpy(&((unsigned char *)addr)[SPX_OFFSET_TREE], tree_bytes, 8);
#else
...
void set_keypair_addr(uint32_t addr[8], uint32_t keypair)
{
    ...
    // Use two bytes for keypair address if needed
